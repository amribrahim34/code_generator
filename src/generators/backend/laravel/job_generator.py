...
class JobGenerator(ICodeGenerator):
    ...
    def generate(self, schema: Schema) -> Dict[str, str]:
        ...
        for model in schema.models:
            job_content = self._generate_job(model)
            model_name = model['name'] if isinstance(model, dict) else model.name
            file_path = f"backend/app/Jobs/{model_name}Job.php"
            generated_files[file_path] = job_content
                
        return generated_files

    def get_output_path(self, model_name: str ) -> str:
        return f"backend/app/Jobs/{model_name}Job.php"

    def render_template(self, template: str, context: Dict[str, Any]) -> str:
        return self.template_renderer.render(template, context)

    def _generate_job(self, model: Union[Dict[str, Any], Model]) -> str:
        context = self.prepare_context(model)
        template = self.get_template('job')
        return self.template_renderer.render('backend/laravel/job.stub', context)
    ...
    def prepare_context(self, model: Union[Dict[str, Any], Model]) -> Dict[str, Any]:
        ...
        return {
            ...
            'namespace': self.config_loader.get('job_namespace', 'App\\Jobs'),
    ...
    def get_template(self, template_name: str) -> str:
        template_path = self.config_loader.get('job_template_path', 'backend/laravel/job.stub')
